...
class Test_Triepuu(unittest.TestCase):
    ...
    def test_luo_lause_test_2_aste_toimii(self):
        ...
        self.assertEqual(tulos2, "{PARAMETRIT: kuusi, 0, 2}:  kuusi neljä viisi ")
        #param lisää 4, aloitusmonikko on 3
        self.assertEqual(len(tulos2.split()), 7)

    def test_epasallittua_monikkoa_ei_generaatiossa(self):
        lopputulos = True
        i = 0
        matchit = ["yksi neljä viisi", "kaksi neljä viisi", "kolme neljä kuusi", "yksi kaksi kuusi"]
        tekstit = []
        while i < 20:
            testi_generaatio = self.ketju.luo_lause_test("yksi",self.testipuu,20,2)
            i = i + 1
